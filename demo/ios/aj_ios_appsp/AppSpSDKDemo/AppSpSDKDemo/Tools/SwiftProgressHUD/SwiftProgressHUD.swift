...
public class SwiftProgressHUD {
    ...
    @discardableResult
    public class func showWaitLoading(_ message: String) -> UIWindow? {
        if let _ = UIApplication.shared.keyWindow {
            return SwiftProgress.waitLoading(message)
        }
        return nil
    }
    ...
    /// Only display text
    @discardableResult
    public class func showOnlyText(_ text: String, _ autoClearTime: TimeInterval = 2) -> UIWindow? {
        if let _ = UIApplication.shared.keyWindow {
            return SwiftProgress.showText(text, autoClear: true, autoClearTime: autoClearTime)
        }
        return nil
    }
...
class SwiftProgress: NSObject {
    ...
    @discardableResult
    static func waitLoading(_ message: String) -> UIWindow {
        let window = UIWindow()
...
//        window.addGestureRecognizer(tapGesture)
        ...
        mainView.snp.makeConstraints { (make) in
            make.width.equalTo(myMAX(size.width + 50, 80))
...
//        window.addGestureRecognizer(tapGesture)
    ...
    @discardableResult
    static func showText(_ text: String, autoClear: Bool=true, autoClearTime: TimeInterval = 2) -> UIWindow {
        let window = UIWindow()
        window.backgroundColor = hudBackgroundColor
        window.rootViewController = UIViewController()
        window.rootViewController?.view.backgroundColor = UIColor.clear
